"""Groq-backed chat for Lumen (same JSON contract as Gemini chat).
...
from lumen.identity import SYSTEM_PROMPT_ENGINE_NOTE_AR, system_identity_line
...
def _build_system(context: dict[str, Any]) -> str:
    ...
    try:
        from lumen.engine.services.tool_runtime.registry import tool_catalog_for_prompt
...
def chat_via_groq(
    ...
    """Lumen chat via Groq. Returns None if disabled/unavailable."""
    try:
