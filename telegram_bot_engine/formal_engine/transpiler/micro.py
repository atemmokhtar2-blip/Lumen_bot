...
def _emit_handlers_module(inf: InferenceResult) -> str:
    ...
    lines.append("        if str(key) != 'confirm' and str(text).replace('.', '', 1).isdigit():")
    lines.append("            try:")
    ...
    lines.append("        return")
    lines.append("    # Catalog item buttons → order flow (quantity → confirm)")
    lines.append("    label = CALLBACK_LABELS.get(data) or data")
    lines.append("    if label and not target_cmd:")
    lines.append("        if 'order' in FLOWS and FLOWS.get('order'):")
    lines.append("            ud = context.user_data")
    lines.append("            ud.clear()")
    lines.append("            ud['flow'] = 'order'")
    lines.append("            ud['step'] = 0")
    lines.append("            ud['data'] = {'item_name': label}")
    lines.append("            ud['state'] = 'flow:order:0'")
    lines.append("            steps = FLOWS.get('order') or []")
    lines.append("            prompt = steps[0]['prompt'] if steps else 'أرسل الكمية'")
    lines.append("            if query.message is not None:")
    lines.append("                await query.edit_message_text(f'اخترت: {label}\n{prompt}')")
    lines.append("            return")
    lines.append("        if query.message is not None:")
