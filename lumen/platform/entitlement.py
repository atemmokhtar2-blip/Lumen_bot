"""Lumen Pro entitlement resolver (platform shared kernel) — secure, server-side, tamper-resistant.
...
logger = logging.getLogger("lumen.platform.entitlement")
...
def resolve_plan_limits(user_id: int) -> PlanLimits:
    ...
    if ent is not None:
        return PlanLimits(
            max_bots=int(PRO_PLAN_BOT_LIMIT),
            disk_mb=int(PRO_PLAN_DISK_MB),
            memory_mb=int(PRO_PLAN_MEMORY_MB),  # shared pool total
            cpu=float(PRO_PLAN_CPU),  # per bot
            is_pro=True,
