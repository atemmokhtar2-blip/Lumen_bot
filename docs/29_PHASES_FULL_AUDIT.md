...
| **منطق منتج** | pytest IDOR, `api/ownership.py`, live_idor_http | نعم — عزل tenants ليس بديل ZAP |
...
| ملف | المهمة |
|-----|--------|
| `api/ownership.py` | reject_identity_spoof, normalize_tenant_id, assert_job_owned |
| `api/routes/jobs|hosts|generate|billing|tenants|audit` | استدعاء الملكية/الـ spoof |
