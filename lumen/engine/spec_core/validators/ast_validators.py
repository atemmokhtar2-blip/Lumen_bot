...
def _ensure_referenced_service_stubs(root: Path, files_written: list[str]) -> list[str]:
    ...
    import ast as _ast
    from lumen.engine.services.safe_fs import safe_write_under_root, safe_ident, UnsafePathError
    notes: list[str] = []
    root = Path(root)
    needed: set[str] = set()
    ...
    def _write(name: str, content: str, tag: str) -> None:
        from lumen.engine.services.safe_fs import safe_ident, safe_write_text, UnsafePathError
        try:
