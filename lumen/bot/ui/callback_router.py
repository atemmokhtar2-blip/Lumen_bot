...
async def _handle_hitl_callback(update, context, q, action_id: str) -> bool:
    """Delegate to callbacks.hitl (split for maintainability)."""
    from lumen.bot.ui.callbacks.hitl import handle_hitl_callback
    return await handle_hitl_callback(update, context, q, action_id)
...
async def _handle_ui_callback_body(update, context, q, action_id: str, arg: str) -> None:
    ...
    # ── Direct engine-bound actions (split module) ──────────
    from lumen.bot.ui.callbacks.direct_actions import handle_direct_actions
    if await handle_direct_actions(
        action_id=action_id,
        arg=arg,
        update=update,
        context=context,
    ...
    # GitHub post-actions (split module)
    from lumen.bot.ui.callbacks.github_actions import handle_github_post_actions
    await handle_github_post_actions(
        action_id=action_id,
        result=result,
        update=update,
        context=context,
