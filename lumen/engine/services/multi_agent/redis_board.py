...
from lumen.identity import REDIS_KEY_PREFIX as _DEFAULT_REDIS_PREFIX
_PREFIX = (os.environ.get("MULTI_AGENT_REDIS_PREFIX") or _DEFAULT_REDIS_PREFIX).strip() or _DEFAULT_REDIS_PREFIX
...
def enqueue_resume_job(state_id: str, *, tenant_id: str = "platform") -> dict[str, Any] | None:
    if not redis_board_enabled():
        return None
    try:
        from lumen.platform.task_queue import enqueue_job
        return enqueue_job(
            tenant_id=tenant_id,
