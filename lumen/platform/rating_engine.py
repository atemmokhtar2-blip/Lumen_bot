...
def _lifecycle_after_rate(tenant_id: str, *, failed_reason: str = "") -> None:
    try:
        from lumen.platform.balance_lifecycle import get_balance_lifecycle
        lc = get_balance_lifecycle()
        if failed_reason:
            lc.on_rating_failure(tenant_id, failed_reason)
        else:
            lc.on_balance_changed(tenant_id)
    except Exception:
        logger.debug("balance lifecycle hook skipped", exc_info=True)
