"""Tolerant data readers for Workflow Execution (Spec 064)."""
...
class SchedulerReader(_BaseReader):
    ARTEFACT_KEY = "task_scheduler_report"
    LIST_KEY = "tasks"


class QueueReader(_BaseReader):
    ARTEFACT_KEY = "message_queue_report"
    LIST_KEY = "messages"
...
class UserRequestReader(_BaseReader):
    ...
    def read(self, ctx: GenerationContext) -> GenericData:
        ...
        try:
            a = ctx.get(self.ARTEFACT_KEY)
            if a is None:
                a = ctx.get("workflow_request")
            if a is None:
                d.error = "missing user_request"
                return d
            raw = _safe(a)
            d.raw, d.available = raw, True
            stages = raw.get("stages") or raw.get("workflow") or raw.get("tasks") or []
