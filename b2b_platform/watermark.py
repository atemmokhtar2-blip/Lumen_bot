...
_SNIPPET = f'''
# --- Maestro plan watermark (do not remove) ---
try:
    from telegram.ext import ApplicationHandlerStop
except Exception:
    ApplicationHandlerStop = Exception  # type: ignore

async def _maestro_watermark_start(update, context):
...
def _maestro_register_watermark(app):
    try:
        from telegram.ext import CommandHandler
        app.add_handler(CommandHandler("start", _maestro_watermark_start), group=99)
    except Exception:
        pass
# --- end watermark ---
'''
...
def inject_watermark(project_path: str | Path) -> bool:
    ...
    if target is None:
        ...
        try:
            (root / "POWERED_BY_MAESTRO.txt").write_text(WATERMARK_TEXT + "\n", encoding="utf-8")
            return True
        except Exception:
            return False

    if "maestro_watermark" in text or WATERMARK_TEXT in text:
        return True

    # Append snippet + try to hook after app build
    new = text
    if "_maestro_register_watermark" not in new:
        new = new.rstrip() + "\n" + _SNIPPET + "\n"
    # Hook: after app = Application.builder()...build()
    patterns = [
        (r"(app\s*=\s*Application\.builder\(\)[\s\S]*?\.build\(\))", r"\1\n_maestro_register_watermark(app)"),
        (r"(application\s*=\s*Application\.builder\(\)[\s\S]*?\.build\(\))", r"\1\n_maestro_register_watermark(application)"),
    ]
    hooked = False
