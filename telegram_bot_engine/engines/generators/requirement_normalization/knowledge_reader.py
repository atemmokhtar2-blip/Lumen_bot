...
class KnowledgeData:
    ...
    common keys that the Requirement Normalization Engine uses, while
    keeping the full dictionary available for custom look-ups.

    Attributes:
        assumptions: The list of pre-approved assumptions.
        defaults: A dictionary of default values keyed by field
            name.
        domain_rules: A list of domain-specific rules.
        constraints: A list of project-level constraints.
        synonyms: A dictionary of synonym mappings (word →
            canonical form).
        abbreviations: A dictionary of abbreviation mappings
            (abbreviation → expanded form).
        terminology: A dictionary of terminology mappings
            (term → canonical term).
...
class KnowledgeReader:
    ...
    domain rules, constraints, synonyms, abbreviations, terminology)
    and keeps the full dictionary.  When absent, it returns a
    :class:`KnowledgeData` with ``available=False``.
    """
