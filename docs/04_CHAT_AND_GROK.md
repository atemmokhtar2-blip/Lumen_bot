...
| الدور | المزود | المفاتيح |
|--------|--------|----------|
| **الشات / التوجيه فقط** | **Groq فقط** | `GROQ_API_KEY` / `GROQ_API_KEYS` |
| **الترجمة / عقد spec** | **Gemini (Google) فقط** | `GEMINI_API_KEY` / `GEMINI_API_KEYS` |

- `TBE_STRICT_LLM_ROLES=1` (افتراضي): لا يختلط المزودان — الشات لا يستخدم Gemini، والترجمة لا تستخدم Groq.
- تدوير المفاتيح يتم **داخل** نفس المزود فقط (مثلاً مفتاح Groq خلص → المفتاح التالي من `GROQ_API_KEYS`).
- ممنوع استخدام Gemini للرد على المستخدم في الشات، وممنوع استخدام Groq لترجمة `spec_core`.
