...
def _enabled() -> bool:
    raw = (os.getenv("GROQ_TRANSLATOR_ENABLED") or os.getenv("LUMEN_TRANSLATOR_ENABLED") or "").strip().lower()
...
def _build_messages(text: str, context: dict[str, Any], capabilities: list[str]) -> list[dict[str, str]]:
    ...
    system = (
        "You are Lumen Spec Translator for Telegram bots. "
    ...
    try:
        from lumen.engine.services.prompt_fence import system_prompt_injection_rules
...
def translate_via_groq(text: str, context: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    try:
        from lumen.engine.services.prompt_fence import sanitize_user_text, system_prompt_injection_rules
...
def translate_infinite_via_gemini(text: str, context: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    try:
        from lumen.engine.services.gemini_client import enabled as gemini_enabled, _available_api_keys, model_name, _timeout
    except Exception:
