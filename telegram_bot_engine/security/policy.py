...
_CONFIRMATION_REQUIRED: Set[str] = {
    "git_push", "create_repo", "repo_modify", "host_start", "host_stop", "terminal_exec", "deploy",
}

class PolicyEngine:
    def __init__(self, *, denied_tools: Optional[Set[str]] = None,
                 confirmation_required: Optional[Set[str]] = None,
                 allowed_tools: Optional[Set[str]] = None) -> None:
    ...
    def evaluate(self, request: ToolRequest) -> PolicyDecision:
        ...
        if self._allowed is not None and name not in self._allowed:
            return PolicyDecision(PolicyVerdict.DENY, f"tool '{name}' not allowed")
        if name in self._confirm and not request.confirmed:
            return PolicyDecision(PolicyVerdict.REQUIRE_CONFIRMATION,
                f"tool '{name}' requires confirmation",
                redacted_params=_redact(request.params))
        return PolicyDecision(PolicyVerdict.ALLOW, "ok", redacted_params=_redact(request.params))
...
__all__ = ["PolicyVerdict", "ToolRequest", "PolicyDecision", "PolicyEngine"]
