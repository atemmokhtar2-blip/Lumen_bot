...
def publish_artifact(local_zip: Path, job_id: str) -> str:
    ...
    """
    try:
        from telegram_bot_engine.services.object_storage import enabled, upload_file
        if enabled():
...
def fetch_artifact(artifact_uri: str, job_id: str) -> Path:
    ...
    if uri.startswith("s3://") or (not uri.startswith("file://") and "://" not in uri and enabled_s3()):
        ...
        if uri.startswith("s3://"):
            ...
            key = uri
        from telegram_bot_engine.services.object_storage import download_file
...
def enabled_s3() -> bool:
    try:
        from telegram_bot_engine.services.object_storage import enabled
        return enabled()
    except Exception:
        return False
