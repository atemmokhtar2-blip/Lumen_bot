...
def _ensure_namespace_packages() -> None:
    ...
    # We intentionally do NOT import lumen.engine.engines (circular).
    # Only ensure namespace modules exist for relative imports inside smart_clone.
    parts = [
        "lumen.engine",
        "lumen.engine.engines",
        "lumen.engine.engines.generators",
        "lumen.engine.engines.generators.git_operations",
    ]
    # lumen.engine is normally already imported
    for name in parts:
        if name in sys.modules:
            continue
        # If real package can be imported without side effects for top-level only
        if name == "lumen.engine":
            continue
        # Create a lightweight namespace package pointing at the directory
        m = ModuleType(name)
        # map path
        rel = name.replace("lumen.engine.", "").replace(".", "/")
        if name == "lumen.engine.engines":
            pkg_path = Path(__file__).resolve().parents[1] / "engines"
        elif name == "lumen.engine.engines.generators":
...
def load_git_op_module(stem: str) -> ModuleType:
    ...
    mod.__package__ = "lumen.engine.engines.generators.git_operations"
    # CRITICAL order
