...
def test_stale_lock_reclaimed_when_pid_dead():
    import bot_interface.singleton as sing

    sing._LOCK_FH = None
    root = tempfile.mkdtemp()
    os.environ["OUTPUT_DIR"] = root
    lock_path = Path(root) / ".ai_agent_7h_bot.poll.lock"
...
def test_default_output_dir_not_tmp_generated():
    ...
    import b2b_platform.paths as paths

    paths._RESOLVED = None
    d = paths.default_output_dir()
    assert "/tmp/generated" not in d
    assert "capability_maestro" in d or ".runtime" in d or str(Path.home()) in d
