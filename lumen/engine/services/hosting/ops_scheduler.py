...
def _backup_interval() -> float:
    try:
        from lumen.engine.services.hosting.backup_manager import interval_hours
...
def _loop(get_service: Callable) -> None:
    ...
    while not _stop.is_set():
        try:
            svc = get_service()
            if svc is not None:
                for inst in list(getattr(svc, "_instances", {}) or {}).values():
                ...
                if now - last_backup >= _backup_interval():
                    try:
                        from lumen.engine.services.hosting.backup_manager import backup_all_running
