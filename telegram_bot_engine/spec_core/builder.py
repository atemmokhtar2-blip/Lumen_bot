...
class BuilderSession:
    ...
    def set_name(self, name: str) -> None:
        ...
        import re as _re
        raw = (name or "").strip()
        # Keep intentional short ids like group_admin_bot / tasks_bot
        if raw and len(raw) <= 32 and raw.replace("_", "").isalnum() and (
            raw.endswith("_bot") or raw in {"my_bot", "basic_bot"}
        ):
            self.bot_name = raw[:40]
            return
        try:
        ...
        if "lang" in selected or "language" in selected:
            features.append(
                Feature(
                    id="lang_cb",
                    feature="lang",
                    trigger=Trigger(type="callback", id="cmd:lang"),
                    action=Action(service="i18n", method="set_lang"),
                    messages=Messages(success="تم تغيير اللغة"),
                )
            )
            start_buttons.append(StartButton(label="🌐 اللغة", callback_id="cmd:lang"))
