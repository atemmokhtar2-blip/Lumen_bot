...
class HostingService:
    ...
    def _save_unlocked(self) -> None:
        ...
        for inst in self._instances.values():
            ...
            # Durable truth = Postgres/SQLite via self._store (already written above).
            # Redis is a hot multi-worker cache only — never abort a successful
            # host start because the cache write failed (missing REDIS_URL,
            # TLS flake, brief outage). Health/ingress rehydrate from DB.
            self._store.upsert(payload)
            try:
                ok_r = host_redis.put_instance(payload)
                if ok_r is False:
                    logger.warning(
                        "host redis cache miss on put instance_id=%s — durable store ok",
                        payload.get("instance_id"),
                    )
            except Exception as exc:
                logger.warning(
                    "host redis cache error on put: %s — durable store ok",
                    type(exc).__name__,
                )
