"""
Git Operations Report (Specification 047 — CRITICAL).

Executes Git operations only after user/permission/repo verification.
Dangerous ops require explicit user confirmation. No autonomous history rewrite.
"""

from __future__ import annotations

from dataclasses import dataclass, field
from typing import Any, Dict, List


SOURCE_REPOSITORY_MANAGEMENT = "repository_management_report"
SOURCE_PROJECT_CONTEXT = "project_context_report"
SOURCE_USER_REQUEST = "user_request"
SOURCE_PRODUCTION_READINESS = "production_readiness_report"

ALL_SOURCES = (
    SOURCE_REPOSITORY_MANAGEMENT,
    SOURCE_PROJECT_CONTEXT,
    SOURCE_USER_REQUEST,
    SOURCE_PRODUCTION_READINESS,
)

SEVERITY_CRITICAL = "critical"
SEVERITY_HIGH = "high"
SEVERITY_MEDIUM = "medium"
SEVERITY_LOW = "low"
SEVERITY_INFO = "info"

# Git operations
OP_CLONE = "git_clone"
OP_INIT = "git_init"
OP_FETCH = "git_fetch"
OP_PULL = "git_pull"
OP_ADD = "git_add"
OP_COMMIT = "git_commit"
OP_PUSH = "git_push"
OP_CHECKOUT = "git_checkout"
OP_SWITCH = "git_switch"
OP_MERGE = "git_merge"
OP_REBASE = "git_rebase"
OP_STASH = "git_stash"
OP_TAG = "git_tag"
OP_RESTORE = "git_restore"
OP_RESET = "git_reset"
OP_CLEAN = "git_clean"
OP_BRANCH_CREATE = "git_branch_create"
OP_BRANCH_RENAME = "git_branch_rename"
OP_BRANCH_DELETE = "git_branch_delete"
OP_FORCE_PUSH = "git_push_force"
OP_RESET_HARD = "git_reset_hard"

ALL_OPERATIONS = (
    OP_CLONE, OP_INIT, OP_FETCH, OP_PULL, OP_ADD, OP_COMMIT, OP_PUSH,
    OP_CHECKOUT, OP_SWITCH, OP_MERGE, OP_REBASE, OP_STASH, OP_TAG,
    OP_RESTORE, OP_RESET, OP_CLEAN, OP_BRANCH_CREATE, OP_BRANCH_RENAME,
    OP_BRANCH_DELETE, OP_FORCE_PUSH, OP_RESET_HARD,
)

# Dangerous operations requiring explicit confirmation
DANGEROUS_OPS = {
    OP_FORCE_PUSH, OP_RESET_HARD, OP_BRANCH_DELETE, OP_CLEAN, OP_RESET,
}

RULE_USER_VERIFIED = "user_verified"
RULE_PERMISSION_OK = "permission_ok"
RULE_REPO_VERIFIED = "repo_verified"
RULE_DANGEROUS_CONFIRMED = "dangerous_ops_confirmed"
RULE_NO_AUTONOMOUS_HISTORY = "no_autonomous_history_rewrite"
RULE_SELF_VERIFICATION = "self_verification_passed"
RULE_QUALITY_PASS = "quality_pass"

ALL_QUALITY_RULES = (
    RULE_USER_VERIFIED,
    RULE_PERMISSION_OK,
    RULE_REPO_VERIFIED,
    RULE_DANGEROUS_CONFIRMED,
    RULE_NO_AUTONOMOUS_HISTORY,
    RULE_SELF_VERIFICATION,
    RULE_QUALITY_PASS,
)

CACHE_HIT = "hit"
CACHE_MISS = "miss"
CACHE_DISABLED = "disabled"

CONFIDENCE_HIGH = "high"
CONFIDENCE_MEDIUM = "medium"
CONFIDENCE_LOW = "low"
CONFIDENCE_HIGH_THRESHOLD = 0.85
CONFIDENCE_MEDIUM_THRESHOLD = 0.60

VERDICT_READY = "ready"
VERDICT_READY_WITH_WARNINGS = "ready_with_warnings"
VERDICT_NOT_READY = "not_ready"
VERDICT_DENIED = "denied"

ALL_VERDICTS = (
    VERDICT_READY, VERDICT_READY_WITH_WARNINGS, VERDICT_NOT_READY, VERDICT_DENIED,
)

STATUS_PLANNED = "planned"
STATUS_EXECUTED = "executed"
STATUS_DENIED = "denied"
STATUS_FAILED = "failed"
STATUS_AWAITING_CONFIRMATION = "awaiting_confirmation"
STATUS_ROLLED_BACK = "rolled_back"


@dataclass
class CommitInfo:
    commit_id: str
    title: str = ""
    description: str = ""
    timestamp: str = ""
    author: str = ""
    modified_files: List[str] = field(default_factory=list)

    def to_dict(self) -> Dict[str, Any]:
        return {
            "commit_id": self.commit_id,
            "title": self.title,
            "description": self.description,
            "timestamp": self.timestamp,
            "author": self.author,
            "modified_files": list(self.modified_files),
        }


@dataclass
class BranchInfo:
    name: str
    action: str = ""  # create|rename|delete|switch|protect
    protected: bool = False
    previous_name: str = ""

    def to_dict(self) -> Dict[str, Any]:
        return {
            "name": self.name,
            "action": self.action,
            "protected": self.protected,
            "previous_name": self.previous_name,
        }


@dataclass
class ConflictInfo:
    conflict_id: str
    conflict_type: str  # merge|rebase|history|file
    files: List[str] = field(default_factory=list)
    suggestion: str = ""
    resolved: bool = False
    user_approved: bool = False

    def to_dict(self) -> Dict[str, Any]:
        return {
            "conflict_id": self.conflict_id,
            "conflict_type": self.conflict_type,
            "files": list(self.files),
            "suggestion": self.suggestion,
            "resolved": self.resolved,
            "user_approved": self.user_approved,
        }


@dataclass
class GitOperation:
    operation_id: str
    operation: str
    status: str = STATUS_PLANNED
    repository: str = ""
    branch: str = ""
    message: str = ""
    dangerous: bool = False
    confirmed: bool = False
    user_id: str = ""
    timestamp: str = ""
    verification_ok: bool = False
    rolled_back: bool = False

    def to_dict(self) -> Dict[str, Any]:
        return {
            "operation_id": self.operation_id,
            "operation": self.operation,
            "status": self.status,
            "repository": self.repository,
            "branch": self.branch,
            "message": self.message,
            "dangerous": self.dangerous,
            "confirmed": self.confirmed,
            "user_id": self.user_id,
            "timestamp": self.timestamp,
            "verification_ok": self.verification_ok,
            "rolled_back": self.rolled_back,
        }


@dataclass
class HistoryEntry:
    entry_id: str
    kind: str  # commit|branch|merge|push
    summary: str = ""
    timestamp: str = ""
    details: Dict[str, Any] = field(default_factory=dict)

    def to_dict(self) -> Dict[str, Any]:
        return {
            "entry_id": self.entry_id,
            "kind": self.kind,
            "summary": self.summary,
            "timestamp": self.timestamp,
            "details": dict(self.details),
        }


@dataclass
class GitFinding:
    severity: str
    code: str
    message: str
    affected: str = ""
    resolution_hint: str = ""
    category: str = "git"

    def to_dict(self) -> Dict[str, Any]:
        return {
            "severity": self.severity,
            "code": self.code,
            "message": self.message,
            "affected": self.affected,
            "resolution_hint": self.resolution_hint,
            "category": self.category,
        }


@dataclass
class CacheInfo:
    status: str = CACHE_MISS
    key: str = ""
    created_at: str = ""
    hits: int = 0

    def to_dict(self) -> Dict[str, Any]:
        return {
            "status": self.status,
            "key": self.key,
            "created_at": self.created_at,
            "hits": self.hits,
        }


@dataclass
class GitProvenance:
    engine_name: str = "git_operations"
    engine_version: str = "1.0.0"
    sources_used: List[str] = field(default_factory=list)
    sources_missing: List[str] = field(default_factory=list)
    generated_at: str = ""
    confidence: float = 0.0
    confidence_level: str = CONFIDENCE_LOW
    self_verification_passed: bool = False

    def to_dict(self) -> Dict[str, Any]:
        return {
            "engine_name": self.engine_name,
            "engine_version": self.engine_version,
            "sources_used": list(self.sources_used),
            "sources_missing": list(self.sources_missing),
            "generated_at": self.generated_at,
            "confidence": self.confidence,
            "confidence_level": self.confidence_level,
            "self_verification_passed": self.self_verification_passed,
        }


@dataclass
class GitOperationsReport:
    report_id: str = ""
    operations: List[GitOperation] = field(default_factory=list)
    commits: List[CommitInfo] = field(default_factory=list)
    branches: List[BranchInfo] = field(default_factory=list)
    conflicts: List[ConflictInfo] = field(default_factory=list)
    history: List[HistoryEntry] = field(default_factory=list)
    findings: List[GitFinding] = field(default_factory=list)
    operation_count: int = 0
    denied_count: int = 0
    failed_count: int = 0
    awaiting_confirmation_count: int = 0
    user_verified: bool = False
    permission_ok: bool = False
    repo_verified: bool = False
    self_verification_passed: bool = False
    readiness_status: str = VERDICT_NOT_READY
    verdict: str = VERDICT_NOT_READY
    cache_info: CacheInfo = field(default_factory=CacheInfo)
    provenance: GitProvenance = field(default_factory=GitProvenance)
    metadata: Dict[str, Any] = field(default_factory=dict)
    is_empty: bool = True

    def to_dict(self) -> Dict[str, Any]:
        return {
            "report_id": self.report_id,
            "operations": [o.to_dict() for o in self.operations],
            "commits": [c.to_dict() for c in self.commits],
            "branches": [b.to_dict() for b in self.branches],
            "conflicts": [c.to_dict() for c in self.conflicts],
            "history": [h.to_dict() for h in self.history],
            "findings": [f.to_dict() for f in self.findings],
            "operation_count": self.operation_count,
            "denied_count": self.denied_count,
            "failed_count": self.failed_count,
            "awaiting_confirmation_count": self.awaiting_confirmation_count,
            "user_verified": self.user_verified,
            "permission_ok": self.permission_ok,
            "repo_verified": self.repo_verified,
            "self_verification_passed": self.self_verification_passed,
            "readiness_status": self.readiness_status,
            "verdict": self.verdict,
            "cache_info": self.cache_info.to_dict(),
            "provenance": self.provenance.to_dict(),
            "metadata": dict(self.metadata),
            "is_empty": self.is_empty,
        }


__all__ = [
    "SOURCE_REPOSITORY_MANAGEMENT", "SOURCE_PROJECT_CONTEXT",
    "SOURCE_USER_REQUEST", "SOURCE_PRODUCTION_READINESS", "ALL_SOURCES",
    "SEVERITY_CRITICAL", "SEVERITY_HIGH", "SEVERITY_MEDIUM", "SEVERITY_LOW", "SEVERITY_INFO",
    "OP_CLONE", "OP_INIT", "OP_FETCH", "OP_PULL", "OP_ADD", "OP_COMMIT", "OP_PUSH",
    "OP_CHECKOUT", "OP_SWITCH", "OP_MERGE", "OP_REBASE", "OP_STASH", "OP_TAG",
    "OP_RESTORE", "OP_RESET", "OP_CLEAN", "OP_BRANCH_CREATE", "OP_BRANCH_RENAME",
    "OP_BRANCH_DELETE", "OP_FORCE_PUSH", "OP_RESET_HARD",
    "ALL_OPERATIONS", "DANGEROUS_OPS",
    "RULE_USER_VERIFIED", "RULE_PERMISSION_OK", "RULE_REPO_VERIFIED",
    "RULE_DANGEROUS_CONFIRMED", "RULE_NO_AUTONOMOUS_HISTORY",
    "RULE_SELF_VERIFICATION", "RULE_QUALITY_PASS", "ALL_QUALITY_RULES",
    "CACHE_HIT", "CACHE_MISS", "CACHE_DISABLED",
    "CONFIDENCE_HIGH", "CONFIDENCE_MEDIUM", "CONFIDENCE_LOW",
    "CONFIDENCE_HIGH_THRESHOLD", "CONFIDENCE_MEDIUM_THRESHOLD",
    "VERDICT_READY", "VERDICT_READY_WITH_WARNINGS", "VERDICT_NOT_READY", "VERDICT_DENIED",
    "ALL_VERDICTS",
    "STATUS_PLANNED", "STATUS_EXECUTED", "STATUS_DENIED", "STATUS_FAILED",
    "STATUS_AWAITING_CONFIRMATION", "STATUS_ROLLED_BACK",
    "CommitInfo", "BranchInfo", "ConflictInfo", "GitOperation", "HistoryEntry",
    "GitFinding", "CacheInfo", "GitProvenance", "GitOperationsReport",
]
