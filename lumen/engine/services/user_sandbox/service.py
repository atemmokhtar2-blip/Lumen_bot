...
class UserSandbox:
    ...
    def is_under_sandbox(self, path: str | Path) -> bool:
        """True only if path resolves strictly inside this user's root (no symlink escape)."""
        try:
            ...
            # Reject if any parent is a symlink pointing outside (best-effort)
            try:
                from lumen.engine.services.linux_path_open import verify_dir_beneath
                verify_dir_beneath(root, target, require_openat2=False)
            except Exception:
                # If verify fails closed on missing path, still trust relative_to
...
def write_token_file(project_dir: str | Path, bot_token: str) -> Path | None:
    ...
    try:
        ...
        if not sealed or not str(sealed).startswith(("enc3:", "enc2:", "enc1:")):
