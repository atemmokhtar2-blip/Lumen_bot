...
def _catalog() -> set[str]:
    try:
        from lumen.engine.services.translator_client import _spec_core_capabilities
...
def analyze_and_prepare(
    ...
    if len([k for k in rules if k not in {"start", "help"}]) == 0:
        try:
            from lumen.engine.services.capability_detection.integration import (
                feature_keys,
                run_detection,
            )
            det_feats = [
    ...
    if tr_engine.startswith("infinite") or has_dyn or tr.get("ok") and tr_engine == "infinite_v1":
        ...
        try:
            from lumen.engine.services.engine_router import decide_engine_mode
