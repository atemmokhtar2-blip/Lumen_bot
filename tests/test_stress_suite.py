...
def test_sanitize_for_storage_strips_paths():
    from lumen.platform.sanitize import sanitize_for_storage
...
def test_token_with_newline_still_matches():
    from lumen.bot.helpers import looks_like_bot_token, normalize_bot_token
...
def test_path_metacharacters_rejected():
    from lumen.bot.sanitize import assert_safe_fs_path
...
def test_job_store_cleanup_and_sanitize(tmp_path, monkeypatch):
    monkeypatch.setenv("OUTPUT_DIR", str(tmp_path))
    from lumen.platform.jobs import JobStore, Job, STATUS_SUCCEEDED
...
def test_help_lists_cannot_do():
    from lumen.bot.capability_boundaries import get_help_text, rejection_message
...
def test_generation_cache_roundtrip(tmp_path, monkeypatch):
    monkeypatch.setenv("OUTPUT_DIR", str(tmp_path))
    from lumen.bot.generation_cache import GenerationCache
