...
def validate_spec(spec: BotSpec) -> ValidationResult:
    ...
    for btn in spec.start_buttons:
        cid = (btn.callback_id or "").strip()
        bound = any(
            f.trigger.type == "callback" and f.trigger.id == cid for f in spec.features
        )
        # cmd:xxx buttons are bound if a matching command feature exists
        if not bound and cid.startswith("cmd:"):
            cmd = cid[4:].strip().lower().replace("-", "_")
            bound = any(
                f.trigger.type == "command"
                and (f.trigger.id or "").lower().replace("-", "_") in {cmd, cmd.replace("_", "")}
                for f in spec.features
            ) or cmd in {"lang", "language", "help", "start"}
        if not bound:
