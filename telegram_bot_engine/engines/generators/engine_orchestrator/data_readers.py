"""Tolerant data readers for Engine Orchestrator (Spec 053)."""
...
class UserRequestReader(_BaseReader):
    ...
    def read(self, ctx: GenerationContext) -> GenericData:
        ...
        try:
            a = ctx.get(self.ARTEFACT_KEY)
            if a is None:
                a = ctx.get("execution_request") or ctx.get("orchestrator_request")
            if a is None:
                d.error = "missing user_request"
                return d
            raw = _safe(a)
            d.raw, d.available = raw, True
            engines = raw.get("engines") or raw.get("execution_order") or []
