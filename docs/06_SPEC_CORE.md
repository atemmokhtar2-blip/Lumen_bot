...
الملف المحوري: `telegram_bot_engine/spec_core/pipeline.py` → `build_from_spec`.
...
من واجهة التيليجرام يتم عبر `bot_interface/generation_flow.py` ومسارات `force_generate` / تأكيد المستخدم.

## Pipeline الأوسع (core)

`telegram_bot_engine/pipeline/` يوفّر مراحل:
