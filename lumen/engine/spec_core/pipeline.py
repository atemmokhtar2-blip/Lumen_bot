...
def build_from_spec(
    ...
    *,
    ...
    # Phase E: optional acceptance gate when caller provides the user request
    if request:
        try:
            from .acceptance_gate import accept_spec
            gate = accept_spec(spec, request)
            if gate.warnings or gate.stripped or gate.injected:
                # non-fatal repairs already applied on *spec*
                pass
        except Exception:
            pass
